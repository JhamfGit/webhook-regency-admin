...
// ================================
// ENDPOINT PARA RECIBIR PROYECTO DESDE N8N
// ================================
app.post('/set-project', async (req, res) => {
  try {
    const { conversationId, proyecto } = req.body;
    ...
    console.log(`📋 Guardando proyecto para conversación ${conversationId}: "${proyecto}"`);

    // Guardar el proyecto en los custom attributes
    await updateConversationAttributes(conversationId, {
      proyecto: proyecto
    });
    ...
    console.error('❌ Error en /set-project:', error.message);
...
app.post('/chatwoot-webhook', async (req, res) => {
  try {
    ...
    if (!currentState && proyecto) {
      console.log('🚀 Iniciando flujo automáticamente (proyecto definido)...');
    ...
    // Si no hay estado ni proyecto, no hacer nada
    if (!currentState) {
